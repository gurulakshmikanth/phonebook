...
class ContactList(APIView):
    ...
    def post(self,request):
        if request.data['name'] is None or request.data['phone_number'] is None:
            return Response(
                {
                    "Error":"Both name and phone number required"
                },
...
class SearchName(APIView):
	def get(self,request):
		...
		profile_contain=Profile.objects.filter(user__username__contains=name)
		contact_start=Contact.objects.filter(name__startswith=name)
		contact_contain=Contact.objects.filter(name__contains=name)
		response=[]
